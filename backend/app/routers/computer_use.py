...
async def computer_use_status(_user=Depends(get_current_user)):
...
async def computer_use_refresh(_user=Depends(get_current_user)):
...
async def computer_use_config(_user=Depends(get_current_user)):
...
async def test_remote(_user=Depends(get_current_user)):
...
async def audit_log(limit: int = 50, _user=Depends(get_current_user)):
