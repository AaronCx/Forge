...
# For Supabase users: (re)generate the combined migration file. The file is
# gitignored — ALWAYS regenerate from ALL migrations (sorted). The old version
# only ran when the file was absent and hard-coded a glob that stopped at the
# 20260312 batch, so it silently omitted every later table and RLS policy.
if [ -d "supabase/migrations" ]; then
  combined="supabase/migrations/combined_all.sql"
  echo "-- Forge: Combined Supabase migrations (for cloud mode)" > "$combined"
  echo "-- AUTO-GENERATED by setup.sh — do not edit; regenerated on every setup." >> "$combined"
  echo "" >> "$combined"
  for f in $(ls supabase/migrations/*.sql 2>/dev/null | grep -v 'combined_all.sql' | sort); do
